        ...
        try:
            ...
            for r in rows:
                if isinstance(r, dict):
                    wo_code = (r.get('wo_code') or '').strip() or '-'
                    ...
                    wo_code = (r[0] or '').strip() or '-'
                    scope_desc = (r[1] or '').strip() or '-'
                    company_desc = (r[2] or '').strip() or '-'
                    project_name = (r[3] or '').strip() or '-'
                    engineer_name = (r[4] or '').strip()
                    engineer_email = (r[5] or '').strip() or '-'
                    task_type = (r[6] or '').strip() or '-'
                    month_no = int(r[7] or 0)
                    hours = float(r[8] or 0)
            ...
            return jsonify({
                "message": "Engagement matrix retrieved successfully",
                "result": out
            }), 200
            ...
            return jsonify({
                "message": "Failed to retrieve engagement matrix",
                "error": str(e),
                "result": []
            }), 500
        ...
        try:
            ...
            sql1 = """
                SELECT
                    ...
                    COALESCE(ce.gross_total_cost, 0.00) AS estimate_high,
                    COALESCE(ce.gross_total_cost_low, 0.00) AS estimate_low,
                    COALESCE(ce.gross_total_cost_avg, 0.00) AS estimate_avg,
...
#===========================================================================
        ...
        try:
            ...
            sql1 = """
                SELECT
                    ...
                    wr.business_unit AS wr_business_unit,
                    wrbu.description AS wr_business_unit_desc,
                    COALESCE(a.cost_type_used, 'High') AS cost_type_used
                FROM work_orders a
                LEFT JOIN business_units bu
                    ON a.business_unit = bu.code
                    AND a.org_code = bu.org_code
            ...
            sql2 = """
                SELECT
                    ...
                    COALESCE(mark_up_amount, 0.00) AS mark_up_amount,
                    COALESCE(mark_up_amount_low, 0.00) AS mark_up_amount_low,
                    COALESCE(mark_up_amount_avg, 0.00) AS mark_up_amount_avg,

                    COALESCE(gross_total_cost, 0.00) AS gross_total_cost,
                    COALESCE(gross_total_cost_low, 0.00) AS gross_total_cost_low,
                    COALESCE(gross_total_cost_avg, 0.00) AS gross_total_cost_avg
                FROM wo_cost_estimates
                WHERE wo_number = %s
                AND org_code = %s
                LIMIT 1
            ...
            if not estimate:
                estimate = {
                    ...
                    "mark_up_percent": 0.0,
                    "mark_up_amount": 0.0,
                    "mark_up_amount_low": 0.0,
                    "mark_up_amount_avg": 0.0,
                    "gross_total_cost": 0.0,
                    "gross_total_cost_low": 0.0,
                    "gross_total_cost_avg": 0.0,
                }
            ...
            # ---------------------------------------------------------
            # 6. COST TYPE SELECTION
            # ---------------------------------------------------------
            cost_type_used = text(getv(wo, 'cost_type_used', 24, 'High'))

            materials_cost = as_float(getv(estimate, 'materials_cost', 0, 0))
            materials_cost_low = as_float(getv(estimate, 'materials_cost_low', 1, 0))
            materials_cost_avg = as_float(getv(estimate, 'materials_cost_avg', 2, 0))
            ...
            total_cost = as_float(getv(estimate, 'total_cost', 8, 0))
            total_cost_low = as_float(getv(estimate, 'total_cost_low', 9, 0))
            total_cost_avg = as_float(getv(estimate, 'total_cost_avg', 10, 0))

            tax = as_float(getv(estimate, 'tax', 11, 0))
            payment_terms = text(getv(estimate, 'payment_terms', 12, ''))
            mark_up_percent = as_float(getv(estimate, 'mark_up_percent', 13, 0))

            mark_up_amount = as_float(getv(estimate, 'mark_up_amount', 14, 0))
            mark_up_amount_low = as_float(getv(estimate, 'mark_up_amount_low', 15, 0))
            mark_up_amount_avg = as_float(getv(estimate, 'mark_up_amount_avg', 16, 0))

            gross_total_cost = as_float(getv(estimate, 'gross_total_cost', 17, 0))
            gross_total_cost_low = as_float(getv(estimate, 'gross_total_cost_low', 18, 0))
            gross_total_cost_avg = as_float(getv(estimate, 'gross_total_cost_avg', 19, 0))

            if cost_type_used.lower() == "low":
                selected_materials_cost = materials_cost_low
                selected_subtotal = total_cost_low
                selected_mark_up_amount = mark_up_amount_low
                selected_gross_total = gross_total_cost_low

            elif cost_type_used.lower() == "average":
                selected_materials_cost = materials_cost_avg
                selected_subtotal = total_cost_avg
                selected_mark_up_amount = mark_up_amount_avg
                selected_gross_total = gross_total_cost_avg

            else:
                selected_materials_cost = materials_cost
                selected_subtotal = total_cost
                selected_mark_up_amount = mark_up_amount
                selected_gross_total = gross_total_cost

            # fallback if old records do not yet have gross totals
            if selected_gross_total <= 0:
                selected_gross_total = selected_subtotal + selected_mark_up_amount

            subtotal_before_markup = selected_subtotal + tax

            final_total = (
                subtotal_before_markup
                if template_type == 'internal'
                else selected_gross_total
            )
            ...
            result = {
                ...
                "cost_summary": {
                    ...
                    "materials_cost": selected_materials_cost,
                    "labor_cost": labor_cost,
                    "equipment_cost": equipment_cost,
                    "overhead_cost": overhead_cost,
                    "contingency_fund": contingency_fund,
                    ...
                    "mark_up_percent": mark_up_percent,
                    "mark_up_amount": selected_mark_up_amount,
                    "gross_total": selected_gross_total,
    ...
    # --- get work order counts by planner user --- #
    @app.route('/getworkordercountsbyuser', methods=['GET'])
    def getworkordercountsbyuser():
        ...
        try:
            ...
            sql = """
                SELECT
                    COUNT(DISTINCT wo.wo_number) AS total_count,

                    COUNT(DISTINCT CASE
                        WHEN wo.status NOT IN ('Completed', 'Cancelled')
                        THEN wo.wo_number
                    END) AS open_count,

                    COUNT(DISTINCT CASE
                        WHEN wo.status IN ('Completed', 'Cancelled')
                        THEN wo.wo_number
                    END) AS completed_count

                FROM work_orders wo
                INNER JOIN work_order_team wot
                    ON wo.wo_number = wot.wo_number
                AND wo.org_code = wot.org_code
                WHERE wo.org_code = %s
                AND wot.user = %s
                AND wot.role = 'planner'
            """
    ...
    # --- get work order list by planner user and filter --- #
    @app.route('/getworkorderlistbyuser', methods=['GET'])
    def getworkorderlistbyuser():
        ...
        try:
            ...
            if filter_type == "open":
                ...
                status_filter = """
                    AND wo.status IN ('Completed', 'Cancelled')
                """
            else:
                status_filter = ""
            ...
            sql = f"""
                SELECT DISTINCT
                    ...
                    wo.wo_code,
                    CONCAT(
                        COALESCE(wr.project_desc, ''),
                        ' ',
                        COALESCE(wr.project_details, '')
                    ) AS wr_description,
                    wo.wo_description AS wo_description,
                    wo.status
                FROM work_orders wo
                ...
                AND wo.org_code = wot.org_code
                LEFT JOIN work_requests wr
                    ON wo.wr_id = wr.wr_id
                AND wo.org_code = wr.org_code
                WHERE wo.org_code = %s
                AND wot.user = %s
                AND wot.role = 'planner'
                {status_filter}
            ...
            for row in rows:
                if isinstance(row, dict):
                    result.append({
                        ...
                        "wo_code": row.get("wo_code"),
                        "wr_description": row.get("wr_description"),
                    ...
                    result.append({
                        "wo_number": row[0],
                        "wo_code": row[1],
                        "wr_description": row[2],
                        "wo_description": row[3],
                        "status": row[4],
                    })
    ...
    @app.route('/getpendingapprovalworkorders', methods=['GET'])
    def getpendingapprovalworkorders():
        ...
        try:
            ...
            sql = """
                SELECT DISTINCT
                    ...
                    wo.wo_code,
                    ar.approval_type AS status
                FROM approval_requests ar
                INNER JOIN work_orders wo
                    ON ar.txn_reference = wo.wo_number
                AND ar.org_code = wo.org_code
                LEFT JOIN work_requests wr
                    ON wo.wr_id = wr.wr_id
                AND wo.org_code = wr.org_code
                WHERE ar.org_code = %s
                AND ar.approval_type IN (
                    'Pending Team Lead Approval',
                    'Pending Overall Lead Approval'
                )
                AND (
                    ar.action_status IS NULL
                    OR ar.action_status = ''
                )
                ORDER BY wo.wo_code DESC
            """
    ...
    # --- get service/scope pie charts from work requests --- #
    @app.route('/getwrservicepiecharts', methods=['GET'])
    def getwrservicepiecharts():

        org_code = request.args.get('org_code')
        group_by = request.args.get('group_by', 'company')  # company or project
        ...
        if group_by not in ['company', 'project']:
            return jsonify({
                "message": "Invalid group_by field. Use company or project.",
                "result": []
            }), 400
        ...
        try:
            ...
            if group_by == 'company':
                sql = """
                    SELECT
                        ...
                        bu.code AS slice_code,
                        COALESCE(bu.description, 'Unassigned') AS slice_label,
                        COUNT(DISTINCT wr.wr_id) AS slice_count
                    FROM services s
                    INNER JOIN requested_services rs
                        ON s.service_id = rs.service_id
                    AND s.org_code = rs.org_code
                    INNER JOIN work_requests wr
                        ON rs.wr_id = wr.wr_id
                    AND rs.org_code = wr.org_code
                    LEFT JOIN business_units bu
                        ON wr.business_unit = bu.code
                    AND wr.org_code = bu.org_code
                    WHERE s.org_code = %s
                    AND s.status = %s
                    GROUP BY
                        s.service_id,
                ...
                sql = """
                    SELECT
                        s.service_id,
                        s.description AS service_desc,
                        wr.project_desc AS slice_code,
                        COALESCE(wr.project_desc, 'Unassigned') AS slice_label,
                        COUNT(DISTINCT wr.wr_id) AS slice_count
                    FROM services s
                    INNER JOIN requested_services rs
                        ON s.service_id = rs.service_id
            ...
            for row in rows:
                if isinstance(row, dict):
                    ...
                    slice_count = row.get("slice_count") or 0
                else:
                    service_id = row[0]
                    service_desc = row[1]
                    slice_code = row[2]
                    slice_label = row[3]
                    slice_count = row[4] or 0
                ...
                grouped[service_id]["slices"].append({
    ...
    # --- check if project_gid exists in any work request --- #
    @app.route('/checkprojectgidexists', methods=['GET'])
    def checkprojectgidexists():

        org_code = request.args.get('org_code', '').strip()
        project_gid = request.args.get('project_gid', '').strip()

        if not org_code:
            return jsonify({
                "success": False,
                "message": "Missing org_code"
            }), 400
        ...
        try:
            ...
            cur.execute("""
                SELECT wr_id, wr_code
                FROM work_requests
                WHERE org_code = %s
                AND project_gid = %s
                LIMIT 1
            """, (org_code, project_gid))
            ...
            if row:
                ...
                if isinstance(row, dict):
                    wr_id = row.get('wr_id')
                    wr_code = row.get('wr_code')

                # Handle normal cursor (tuple/list)
                else:
                    wr_id = row[0]
                    wr_code = row[1]
    ...
    # =========================================================
    @app.route('/getservicedetailsbyserviceid', methods=['GET'])
    def getservicedetailsbyserviceid():

        service_id = request.args.get('service_id')
        org_code = request.args.get('org_code')

        if not service_id:
            return jsonify({"message": "service_id is required", "result": []}), 400
